...
class ExploratoryVQDSolver(BaseSolver):
    """Exploratory VQD skeleton delegating to QCchem VQE."""
        ...
        metadata.update(
            {
                ...
                "validation_scope": "vqd skeleton",
                "scientific_risk_notes": [
                    "Current VQD path delegates to QCchem VQE and does not validate excited-state energies.",
                ],
...
def build_solver(spec, backend, seed, problem_summary=None, mapper=None) -> BaseSolver:
    ...
    return ExploratoryVQDSolver(delegate)
