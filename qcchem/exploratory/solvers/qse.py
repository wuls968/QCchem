...
class ExploratoryQSESolver(BaseSolver):
    """Exploratory QSE skeleton delegating to exact diagonalization."""
        ...
        metadata.update(
            {
                ...
                "validation_scope": "qse skeleton",
                "scientific_risk_notes": [
                    "Current QSE path delegates to exact diagonalization and does not validate subspace construction.",
                ],
...
def build_solver(spec, backend, seed, problem_summary=None, mapper=None) -> BaseSolver:
    ...
    return ExploratoryQSESolver(delegate)
